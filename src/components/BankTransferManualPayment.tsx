...
import { CreditCard, Copy, CheckCircle, AlertCircle, MessageCircle, ExternalLink, Building } from "lucide-react";

interface BankTransferManualPaymentProps {
  checkoutData: {
    eventId: string;
    tickets: Array<{
      ticketTypeId: string;
...
const BankTransferManualPayment = ({ 
  checkoutData, 
  customerInfo, 
  onSuccess, 
  onError, 
  disabled = false 
}: BankTransferManualPaymentProps) => {
  ...
  // Bank account and WhatsApp numbers from event data
  ...
  // Fetch event data to get bank account number
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('WhatsApp number from event:', eventData.organizerWhatsApp);
          
          const bankAccount = eventData.accountNumber || "1234567890123456";
          const whatsappNumber = eventData.organizerWhatsApp || "+230 5XXX XXXX";
          
          console.log('Setting account number to:', bankAccount);
  ...
  const handleSubmit = async () => {
    if (!paymentConfirmed) {
      toast({
        ...
        description: "Please confirm that you have completed the bank transfer",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="flex items-center justify-between p-3 sm:p-4 lg:p-6 bg-white border-2 border-gray-200 rounded-lg hover:border-black transition-all duration-200 cursor-pointer"
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 lg:space-x-4 min-w-0 flex-1">
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
              <Building className="h-5 w-5 sm:h-6 sm:w-6 text-black shrink-0" />
                ...
                <div className="space-y-4 sm:space-y-6 lg:space-y-8">
                  <div className="flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-3 lg:space-x-6 p-3 sm:p-4 lg:p-6 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-black mb-2 sm:mb-3 lg:mb-4 text-sm sm:text-base lg:text-lg">Transfer to Bank Account</h4>
                      <div className="space-y-3 sm:space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-blue-50 rounded-lg border-2 border-blue-200 gap-2">
                          <span className="text-sm sm:text-base text-blue-700 font-medium">Transfer to account number:</span>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-xl sm:text-2xl text-blue-800 break-all font-mono">{accountNumber}</span>
                            <Button
                              variant="ghost"
                              size="sm"
                        ...
                        {/* Reference */}
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-yellow-50 rounded-lg border-2 border-yellow-200 gap-2">
                          <span className="text-sm sm:text-base text-yellow-700 font-medium">Use this reference:</span>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-lg sm:text-xl text-yellow-800 break-all font-mono">{referenceCode}</span>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => copyToClipboard(referenceCode, 'Reference code')}
                              className="p-1 h-auto text-yellow-600 hover:text-yellow-800 hover:bg-yellow-100"
                            >
                              <Copy className="h-4 w-4" />
                            </Button>
                          </div>
                        ...
                        <div className="text-sm sm:text-base text-gray-600 leading-relaxed">
                          Use your banking app or visit a branch to transfer the exact amount to the account above. Include the reference code in the transfer description.
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        <div className="flex items-start space-x-3 sm:space-x-4">
                          ...
                          <Label htmlFor="payment-confirmation" className="text-sm sm:text-base text-gray-700 leading-relaxed cursor-pointer">
                            I confirm that I have completed the bank transfer and sent the payment details via WhatsApp
                          </Label>
                        </div>
          ...
          <Card className="border-0 shadow-xl bg-white">
            <CardContent className="p-6">
              <div className="text-center">
                ...
                <p className="text-gray-700 mb-4">Your payment is being processed</p>
                
                <div className="flex items-center justify-center space-x-2 mb-6">
                  <div className="w-3 h-3 bg-black rounded-full animate-pulse"></div>
                  <span className="text-sm text-gray-700 font-medium">Verifying bank transfer...</span>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  ...
                  <div className="space-y-2 text-sm text-left">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-gray-700"><strong>Verification:</strong> Usually within 2-4 hours</span>
                    </div>
                    <div className="flex items-center space-x-3">
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-800">
                    💡 <strong>Pro tip:</strong> Bank transfers may take longer to verify than mobile payments. Keep your WhatsApp and email notifications on!
                  </p>
                </div>
              </div>
            </CardContent>
...
export default BankTransferManualPayment;
